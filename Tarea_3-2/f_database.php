<?php
	class Database{
		...
		public function create($id, $nombre, $posicion, $carrera, $email){
			...
			$stmt = $this->con->prepare("INSERT INTO `futbolistas` (id, nombre, posicion, carrera, email) VALUES ($id, '$nombre', '$posicion', '$carrera', '$email')");
		...
		public function read(){
			// Se delcara la consulta
			$stmt = $this->con->prepare('SELECT * FROM futbolistas');
			// Se carga el resultado de la consulta 
			$stmt->execute();
			return $stmt;
		}
		...
		public function single_record($id){
			// Se delcara la consulta
			$stmt = $this->con->prepare("SELECT * FROM futbolistas where id='$id'");
			// Se carga el resultado de la consulta 
			$stmt->execute();
		...
		public function update($id, $nombre, $posicion, $carrera, $email){
			// Se delcara la consulta
			$stmt = $this->con->prepare("UPDATE futbolistas SET nombre='$nombre', posicion='$posicion', carrera='$carrera', email='$email'  WHERE id=$id");
		...
		public function delete($id){
			// Se delcara la consulta
			$stmt = $this->con->prepare("DELETE FROM futbolistas WHERE id=$id");
			// Se ejecuta la consulta 
			$stmt->execute();
			if($stmt){
				return true;
